...
There is a second number underneath the meter, and it is the one that decides whether the feature survives its first budget review. Cost per call is what the provider bills and what the dashboard graphs, but nobody buys calls. They buy finished work, and the distance between the two is where the money actually is: the retries, the attempts that produced nothing usable and sent the task back to a person, and the outputs that technically succeeded but needed fifteen minutes of editing before anyone could send them. Put the denominator on tasks that genuinely landed, put the human correction time in the numerator at a loaded rate rather than a salary, and compare the result against whatever the workflow replaced. That is cost per successful task, and it is the honest unit of an AI feature's economics. It also tends to embarrass the roadmap. On any workflow with a person reviewing the output, model spend is usually a few percent of the total, so the quarter spent migrating to a model ten times cheaper moves the real number by almost nothing, while the unglamorous work of cutting how often a human has to intervene moves it by half. The reason teams get this backwards is not stupidity, it is instrumentation: the model bill arrives as an invoice and the review time arrives as nothing at all, and unmeasured costs lose arguments to measured ones. Measure the review time and the argument changes.
...
Three scale concerns round out the lab. **Rate limiting** (a per-tenant token bucket) keeps one client from starving a shared, costly backend; with per-request costs this high, fairness and cost control are the same feature. **Model routing** sends easy questions to a cheap model and reserves the expensive one for hard cases, which is often the single largest bill reduction available without touching quality where it matters. And the **feedback loop**. Capture thumbs up and down, because production traffic is the best eval dataset you will ever get, and every thumbs-down is a gold-set case and eventually fine-tuning data. That closes the loop from Chapter 5: production feeds the evals that gate production.
